...
load_data <- function(file = 'jg_iter_data.csv') {
  abm_df <<- read_csv(file)[,-1] %>%
    mutate(simulation = ordered(simulation),
           #aw = as.numeric(aw == 'True'), bethma = as.numeric(bethma == 'True'),
...
plot_agrowell_vs_bethma = function(risk_model = c('eu', 'regret', 'prospect', 'mixed')) {
  ...
  if (risk_model == 'eu') {
    load_data('ranaw_eu_iter_data.csv')
    title <- "Risk-averse expected utility"
    figure_name <- "ranaw_eu_bethma_vs_agrowell.pdf"
  }
  if (risk_model == 'regret') {
    load_data('ranaw_regret_iter_data.csv')
    title <- "Regret-averse expected utility"
    figure_name <- "ranaw_regret_bethma_vs_agrowell.pdf"
  }
  if (risk_model == 'prospect') {
    load_data('ranaw_prospect_iter_data.csv')
    title <- "Prospect theory loss-aversion"
    figure_name <- "ranaw_prospect_bethma_vs_agrowell.pdf"
  }
  if (risk_model == 'mixed') {
    load_data('ranaw_mixed_iter_data.csv')
    title <- "Mixed risk models"
    figure_name <- "ranaw_mixed_bethma_vs_agrowell.pdf"
  }
...
grand_plot = function(save_plot = FALSE, grand_sdf = NULL, grand_df = NULL) {
  ...
  if (save_plot) {
    ggsave('ranaw_grand_abm_bethma.pdf', plot = p1, device = 'pdf', width = 6, height = 4)
    ggsave('ranaw_grand_abm_bethma.png', plot = p1, device = 'png', width = 6, height = 4, dpi = 600)
  }

  p2 <- ggplot(grand_df, aes(x = iter, y = profit, color = agrowell)) +
    geom_jitter(width = 0.2, height = 0, alpha = 0.05, size = 0.3) +
    geom_smooth() +
    scale_color_brewer(palette = 'Dark2', name = "Agrowell") +
  ...
  if (save_plot) {
    ggsave('ranaw_grand_abm_profit.pdf', plot = p2, device = 'pdf', width = 6, height = 4)
    ggsave('ranaw_grand_abm_profit.png', plot = p2, device = 'png', width = 6, height = 4, dpi = 600)
  }
