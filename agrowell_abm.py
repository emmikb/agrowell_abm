...
# True or False
# If True, then initialize high-SES farmers with random duration
# If False, then initialize high-SES farmers with duration = Farmer.agrowell_term
RANDOMIZE_INITIAL_DURATION = False
UTILITY_REFERENCE_SCALE = 120000 #max rupees for payoff
...
class CropForecast(object):
    ...
    def eu_crra(self):
        u = crra_u(self.rp.r, self.w)
        return np.dot(self.p, u) * self.rp.crra_scale

    #regret aversion
    def regret(self):
        u = crra_u(self.rp.rr, self.w)
        u_max = max(u)
        delta_u = u_max - u
        g = 1 - np.power(self.rp.b, delta_u)
        return np.dot(self.p, u - self.rp.k * g) * self.rp.regret_scale
...
class RiskParameters(object):
    def __init__(self, r=1, rr=1, k=0.155, b=0.5, alph=0.88,
        ...
        ref_scale = np.log(UTILITY_REFERENCE_SCALE)
        self.crra_scale =  ref_scale / crra_u(r, UTILITY_REFERENCE_SCALE)
        self.regret_scale = ref_scale / crra_u(rr, UTILITY_REFERENCE_SCALE)
        self.prospect_scale = ref_scale / np.power(UTILITY_REFERENCE_SCALE, alph)
...
class Farmer(object):
    ...
    def __init__(self, farmer_id, payoff, risk_model, risk_parameters,
       ...
       """
        ...
        if agrowell:
            if RANDOMIZE_INITIAL_DURATION:
                duration = random.randint(0,Farmer.agrowell_term)
            else:
                duration = Farmer.agrowell_term
            self.build_agrowell(duration)
...
class Simulation(object):
    ...
    def simulate(self, number_seasons):
        ...
        for s in range(number_seasons):
            ...
            n_agrowells = np.sum([f.agrowell for f in self.farmers])

            print "Running iteration " + str(s) + ": " + \
                str(n_agrowells) + " farmers have agrowells."
